  ...
  return (
    <Content className="carousel">
      ...
      <div className="carouselItems" ref={carouselList}>
        {props.children}
        {props.name === 'videos' && props.data.length === 0
          ? <Empty description="Your teams haven't shared any videos yet" />
          : props.data.map((item) => (
      ...
      <Button
        onClick={() => { scroll(1); }}
        disabled={!!(carouselList.current
          && axis === carouselList.current.scrollWidth - carouselList.current.clientWidth)}
