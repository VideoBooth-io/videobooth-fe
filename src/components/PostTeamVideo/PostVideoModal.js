...
export const PostVideoModal = ({
  toggleStreamPlayback, showModal, toggleModal, videoStream, user_id, promptId, uploadVideo,
}) => {
  ...
  const PostVideoMode = () => {
    if (videoStream && videoStream.playback) {
      return 'playback';
    } if (videoStream && !videoStream.playback && videoStream.raw && videoStream.raw[0] instanceof Blob === true) {
      return 'upload';
    }
    return 'recording';
  };
  ...
  return (
    <Modal
      ...
      okButtonProps={{ form: 'upload', htmlType: 'submit', disabled: PostVideoMode() === 'recording' }}
